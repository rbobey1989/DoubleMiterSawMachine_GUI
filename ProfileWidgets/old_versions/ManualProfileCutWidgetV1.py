...
class ManualProfileCutWidget(Gtk.DrawingArea):
    ...
    def on_draw(self, widget, event):        
        ...
        HEIGHT_ARROW =  HEIGHT_PROF*0.7
        WIDTH_ARROW = HEIGHT_ARROW/1.5
    ...
    def update_length_profile_string_and_value(self, side):
        if side == 'bottom':
            ...
            if self.topLengthProfileString:                
                ...
                if int(self.topLengthProfile) < self.minLength + math.tan(math.radians(90 - self.lefTipAngle))*self.heightProfile + math.tan(math.radians(90 - self.rightTipAngle))*self.heightProfile:
                    self.topLengthProfileString = str(round(self.minLength + math.tan(math.radians(90 - self.lefTipAngle))*self.heightProfile + math.tan(math.radians(90 - self.rightTipAngle))*self.heightProfile,2))
                    self.topLengthProfileString,self.topLengthProfile = self.format_length_string(self.topLengthProfileString)
