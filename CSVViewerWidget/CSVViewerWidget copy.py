...
class CSVViewerWidget(Gtk.Box):
    def __init__(self, barWidget=None, dxfViewerWidget=None):
        ...
        self.treestore = Gtk.TreeStore(bool, GdkPixbuf.Pixbuf, str, str, str, str, str, str, str)  # Adjusted
        self.treeview.set_model(self.treestore)
        # Connect the signal handler to the 'cursor-changed' signal
        self.treeview.connect("cursor-changed", self.on_cursor_changed)

        # Add a CellRendererToggle for the first checkbox
        ...
        profileFields = ['Profile', 'Bar Length', 'Bar Num', 'Length', 'Right Angle', 'Left Angle', 'Description']
    ...
    def validate_csv_format(self,filename):
        with open(filename, 'r') as f:
            reader = csv.reader(f)
            for row in reader:
                if len(row) != 8:  # Check the number of columns
                ...
                if not (isinstance(row[1], str) or row[1] == ''):  # Check the type of column 2
                    ...
                    return False
                if not (self.is_float(row[2]) or row[2] == ''):  # Check the type of column 3
                    print('third field error')
                    return False
                if not (row[3].isdigit() or row[3] == ''):  # Check the type of column 4
                    return False
                if not (self.is_float(row[4]) or row[4] == ''):  # Check the type of column 5
                    return False
                if not (self.is_float(row[5]) or row[5] == ''):  # Check the type of column 6
                    return False
                if not (self.is_float(row[6]) or row[6] == ''):  # Check the type of column 7
                    return False
                if not (isinstance(row[7], str) or row[7] == ''):  # Check the type of column 8
                    return False
        return True
    ...
    def write_rows(self, writer, tree, parent=None):
        ...
        while child_iter is not None:
            ...
            writer.writerow([tree.get_value(child_iter, i) for i in range(tree.get_n_columns())])
    ...
    def load_csv(self, filename):
        ...
        with open(filename, 'r') as f:
            ...
            for row in reader:
                ...
                if row[1]:
                    parent = self.treestore.append(None, [row[0], pixbuf, row[1], row[2], '', '', '', '', ''])  # Adjusted
                else:
                    self.treestore.append(parent, [row[0], pixbuf, '', '', row[3], row[4], row[5], row[6], row[7]])  # Adjusted
...
#     window.main()
    ...
    # def on_open_clicked(self, widget, event):
    ...
    #     if response == Gtk.ResponseType.OK:
    #         self.treestore.clear()
    #         if not self.validate_csv_format(fileChooserDialog.get_filename()):
    #             error_dialog = Gtk.MessageDialog(self, 0, Gtk.MessageType.ERROR,
    #                 Gtk.ButtonsType.OK, "Error")
    #             error_dialog.format_secondary_text(
    #                 f"Error: The file {fileChooserDialog.get_filename()} does not have the correct CSV format.")
    ...
    #         else:
    #             print(f"The file {fileChooserDialog.get_filename()} has the correct CSV format.")
    #             self.load_csv(fileChooserDialog.get_filename())
    ...
    #     fileSavserDialog = Gtk.FileChooserDialog(title="Por favor elige un archivo", parent=self)
    #     fileSavserDialog.add_buttons(Gtk.STOCK_CANCEL, Gtk.ResponseType.CANCEL, Gtk.STOCK_OPEN, Gtk.ResponseType.OK)

    #     response = fileSavserDialog.run()
    #     if response == Gtk.ResponseType.OK:
    #         with open(fileSavserDialog.get_filename(), 'w') as f:
    #             writer = csv.writer(f)
    #             self.write_rows(writer, self.treestore)

    #     fileSavserDialog.destroy()
    ...
    #     self.on_delete_clicked(None, None)
