...
class CSVViewerWidget(Gtk.Notebook):
    def __init__(self, barWidget=None, max_angle = 90, min_angle = 22.5, max_length = 6500, min_length = 240):
        ...
        self.barNumberEntry = Gtk.Entry(name = 'entryCsvCutData')
        self.barNumberEntry.connect("focus-in-event", self.on_focus_in_event_bar_number)
        self.barNumberEntry.connect("focus-out-event", self.on_focus_out_event_bar_number)

        topLengthLabel = Gtk.Label(label='Top Length', name='labelIdicatorsEntryCsvCutData')
        self.topLengthEntry = Gtk.Entry(name = 'entryCsvCutData')
        self.topLengthEntry.connect("focus-in-event", self.on_focus_in_event_top_length)
        self.topLengthEntry.connect("focus-out-event", self.on_focus_out_event_top_length)

        self.topLengthEntry.do_insert_text = self.do_insert_text_top_length.__get__(self.topLengthEntry, Gtk.Entry)

        bottomLengthLabel = Gtk.Label(label='Bottom Length', name='labelIdicatorsEntryCsvCutData')
        self.bottomLengthEntry = Gtk.Entry(name = 'entryCsvCutData')
        self.bottomLengthEntry.connect("focus-in-event", self.on_focus_in_event_bottom_length)
        self.bottomLengthEntry.connect("focus-out-event", self.on_focus_out_event_bottom_length)

        heightLabel = Gtk.Label(label='Height', name='labelIdicatorsEntryCsvCutData')
        self.heightEntry = Gtk.Entry(name = 'entryCsvCutData')
        self.heightEntry.connect("focus-in-event", self.on_focus_in_event_height)
        self.heightEntry.connect("focus-out-event", self.on_focus_out_event_height)

        rightAngleLabel = Gtk.Label(label='Right Angle', name='labelIdicatorsEntryCsvCutData')
        self.rightAngleEntry = Gtk.Entry(name = 'entryCsvCutData')
        self.rightAngleEntry.connect("focus-in-event", self.on_focus_in_event_right_angle)
        self.rightAngleEntry.connect("focus-out-event", self.on_focus_out_event_right_angle)

        leftAngleLabel = Gtk.Label(label='Left Angle', name='labelIdicatorsEntryCsvCutData')
        self.leftAngleEntry = Gtk.Entry(name = 'entryCsvCutData')
        self.leftAngleEntry.connect("focus-in-event", self.on_focus_in_event_left_angle)
        self.leftAngleEntry.connect("focus-out-event", self.on_focus_out_event_left_angle)
        ...
        gridCutData.attach(self.topLengthEntry, 3, 0, 1, 1)

        gridCutData.attach(bottomLengthLabel, 2, 1, 1, 1)
        gridCutData.attach(self.bottomLengthEntry, 3, 1, 1, 1)
        ...
        gridCutData.attach(DescriptionLabel, 0, 2, 1, 2)
        gridCutData.attach(self.DescriptionEntry, 1, 2, 5, 2)
        ...
        with open(filename, 'r') as f:
            ...
            for index,row in enumerate(reader):
                ...
                if not (row[14] in ['0','1'] or row[14] == ''): # Check the type of column 15
                    ...
                    return False
                if not (row[15].isdigit() or row[15] == ''):  # Check the type of column 16
                    LogViewer().emit('public-msg', 'error', 'Error: Sixteenth Field Error At Row ' + str(index + 1) + ': '+ row[15])
                    return False
                if not (isinstance(row[16], str) or row[16] == ''):  # Check the type of column 11
                    LogViewer().emit('public-msg', 'error', 'Error: Seventeenth Field Error At Row ' + str(index + 1) + ': '+ row[16])
                    return False
    ...
    def on_add_profile_csv_list(self, widget):
        if self.dxfViewerWidget != None:
            ...
            if manufacturer != None and set != None and code != None:
                self.treestore.append(None, [True, self.checked_pixbuf, manufacturer, set, code, '5000', '', '', '', '', '', '', '', '', '', '', '', ''])
            else:
                self.treestore.append(None, [True, self.checked_pixbuf, 'DEFINE', 'BY', 'USER', '5000', '', '', '', '', '', '', '', '', '', '', '', ''])
        ...
        if model is not None and iter is not None:
            if model[iter][2:5] != ['', '', '']:
                ...
                if model.iter_has_child(iter):
                    ...
                    self.barNumberEntry.set_text(str(int(model.get_value(last_child_iter, 6)) + 1))
                else:
                    self.barNumberEntry.set_text('1')

                self.set_current_page(self.csvViewerWidgetPages['addCutCsvListPage'])
            
                #self.treestore.append(iter, [True, self.checked_pixbuf, '', '', '', '', '', ' ⬆ ', '', '', '', '', '', '0', '', '', '', ''])        
    ...
    def on_key_pressed(self, widget, key):
        ...
        # self.treeview.queue_draw()     
            ...
            #     print(parent_row)
                # parent_row.pop(1)
                # row = [model.get_value(iter, i) for i in range(model.get_n_columns())]
                # row.pop(1)
                # if row[1] or (row[1].lower() == 'define' and row[2].lower() == 'by' and row[3].lower() == 'user'):
                #     LogViewer().emit('public-msg', 'error', 'Error: The selected row is not a profile.')
                # else:
                #     if row[6] == ' ⬆ ':

    def on_focus_in_event_bar_number(self, widget, event):
        self.entryWithFocus = widget

    def on_focus_out_event_bar_number(self, widget, event):
        self.entryWithFocus = None

    def on_focus_in_event_top_length(self, widget, event):
        self.entryWithFocus = widget

    def on_focus_out_event_top_length(self, widget, event):
        self.entryWithFocus = None

    def do_insert_text_top_length(self, new_text, length, position):
        print('do_insert_text_top_length')
        buffer = self.get_buffer()
        current_text = buffer.get_text(buffer.get_start_iter(), buffer.get_end_iter(), True)
        new_text = current_text[:position] + new_text + current_text[position:]

        if self.validate_fixed_point(new_text, 2):
            buffer.insert_text(position, new_text, length)
            return position + length

        buffer.insert_text(position, new_text[:-1], length - 1)
        return position

    def validate_fixed_point(self, input_string, decimals):
        pattern = r"^\d+(\.\d{0,%d})?$" % decimals
        return re.match(pattern, input_string)
    
    def on_focus_in_event_bottom_length(self, widget, event):
        self.entryWithFocus = widget

    def on_focus_out_event_bottom_length(self, widget, event):
        self.entryWithFocus = None

    def on_focus_in_event_height(self, widget, event):
        self.entryWithFocus = widget

    def on_focus_out_event_height(self, widget, event):
        self.entryWithFocus = None

    def on_focus_in_event_right_angle(self, widget, event):
        self.entryWithFocus = widget

    def on_focus_out_event_right_angle(self, widget, event):
        self.entryWithFocus = None

    def on_focus_in_event_left_angle(self, widget, event):
        self.entryWithFocus = widget

    def on_focus_out_event_left_angle(self, widget, event):
        self.entryWithFocus = None
